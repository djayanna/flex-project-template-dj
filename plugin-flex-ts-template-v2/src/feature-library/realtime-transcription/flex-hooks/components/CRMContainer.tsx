...
import RealTimeTranscriptionView from '../../custom-components/RealTimeTranscriptionView';
import { FlexComponent } from '../../../../types/feature-loader';
// import { getUILocation, isEnhancedCRMEnabled } from '../../config';

export const componentName = FlexComponent.CRMContainer;
export const componentHook = function addRealTimeTranscriptionCRMContainer(flex: typeof Flex, _manager: Flex.Manager) {
  const options: Flex.ContentFragmentProps = {
    if: (props: any) => {
      ...
      if (props.task) {
        ...
        return Flex.TaskHelper.isLiveCall(props.task) && !Flex.TaskHelper.isInWrapupMode(props.task);
      }
      if (props.selectedTaskSid && props.tasks.size > 0) {
        const selectedTaskSid = props.tasks.get(props.selectedTaskSid);
        console.log('task', props.selectedTaskSid);
        return Flex.TaskHelper.isLiveCall(selectedTaskSid);
      }
      return false;
    },
    sortOrder: -1,
  ...
  flex.CRMContainer.Content.replace(<RealTimeTranscriptionView key="realtime-transcription-crm-container" />, options);
  // }
};
